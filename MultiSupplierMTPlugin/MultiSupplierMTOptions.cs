...
namespace MultiSupplierMTPlugin
{
    ...
    class MultiSupplierMTGeneralSettings
    {
        ...
        public string GatewayBaseUrl { get; set; } = DesktopContract.DefaultGatewayBaseUrl;
        public int GatewayTimeoutMs { get; set; } = 120000;
        public bool GatewayFallbackToBuiltinProvider { get; set; } = false;

        public string CurrentServiceProvider { get; set; } = ServiceNames.GatewayDesktop_LLM;
        ...
        public string DataDir { get; set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "MemoQ", "Plugins", "MemoQ.AIGateway");
        ...
        public string[] EnableProviders { get; set; } = new string[]
        {
            ...
            ServiceNames.GatewayDesktop_LLM,
        ...
        public void NormalizeForDesktopControlPlane()
        {
            EnableGateway = true;
            CurrentServiceProvider = DesktopContract.DesktopProviderName;
            GatewayBaseUrl = string.IsNullOrWhiteSpace(GatewayBaseUrl) ? DesktopContract.DefaultGatewayBaseUrl : GatewayBaseUrl;
            GatewayFallbackToBuiltinProvider = false;
            EnableCache = false;
            EnableStatsAndLog = false;
            EnableCustomDisplayName = false;
            EnableCustomRequestLimit = false;

            if (EnableProviders == null || EnableProviders.Length == 0)
            {
                EnableProviders = new[] { DesktopContract.DesktopProviderName };
                return;
            }

            if (!EnableProviders.Contains(DesktopContract.DesktopProviderName))
            {
                EnableProviders = EnableProviders.Concat(new[] { DesktopContract.DesktopProviderName }).ToArray();
            }
        }
    }
    ...
    class SettingsTypeMapping
    {
        public static Dictionary<string, Type> General = new Dictionary<string, Type>
        {
            ...
            {ServiceNames.GatewayDesktop_LLM, typeof(Providers.OpenAI.GeneralSettings)},
        ...
        public static Dictionary<string, Type> Secure = new Dictionary<string, Type>
        {
            ...
            {ServiceNames.GatewayDesktop_LLM, typeof(Providers.OpenAI.SecureSettings)},
